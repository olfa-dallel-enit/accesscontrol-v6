...
func networkWithForwardPolicyObjects(t *testing.T) (*network.Network, types.ForwardPolicy) {
	t.Helper()
	...
	state.ForwardPolicy = forwardPolicy
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), *state.ForwardPolicy
}

func TestShowForwardPolicy(t *testing.T) {
	net, obj := networkWithForwardPolicyObjects(t)
