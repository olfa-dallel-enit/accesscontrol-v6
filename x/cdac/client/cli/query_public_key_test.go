...
func networkWithPublicKeyObjects(t *testing.T, n int) (*network.Network, []types.PublicKey) {
	t.Helper()
	...
	for i := 0; i < n; i++ {
		...
		state.PublicKeyList = append(state.PublicKeyList, publicKey)
	}
	buf, err := cfg.Codec.MarshalJSON(&state)
	require.NoError(t, err)
	cfg.GenesisState[types.ModuleName] = buf
	return network.New(t, cfg), state.PublicKeyList
}
...
func TestListPublicKey(t *testing.T) {
	net, objs := networkWithPublicKeyObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListPublicKey(), args)
			require.NoError(t, err)
			var resp types.QueryAllPublicKeyResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
