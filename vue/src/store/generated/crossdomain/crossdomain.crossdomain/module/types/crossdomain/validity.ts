...
export const Validity = {
  encode(message: Validity, writer: Writer = Writer.create()): Writer {
    if (message.id !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): Validity {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseValidity } as Validity;
  ...
  toJSON(message: Validity): unknown {
    const obj: any = {};
  ...
  fromPartial(object: DeepPartial<Validity>): Validity {
    const message = { ...baseValidity } as Validity;
    if (object.id !== undefined && object.id !== null) {
